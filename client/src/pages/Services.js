    ...
    const handleSubmit = async () => {
        try {
            ...
            console.log(
                "Server Response:",
                Object.entries(
                    JSON.parse(response.data.prediction).weights
                ).map(([label, value]) => ({
                    label,
                    value,
                }))
            );
            const response_green = await axios.post(
                "http://localhost:5000/greenidx",
                {
                    assets: selectedItems,
                }
            );
            setGreenIndex(response_green.data.prediction);
